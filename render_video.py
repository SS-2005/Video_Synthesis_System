...
class Explainer(Scene):
    def construct(self):
        ...
        for scene_idx, scene in enumerate(script["scenes"], 1):
            ...
            # ---------- NARRATION TEXT (Properly wrapped) ----------
            narration_text = scene["narration"]
            
            # Clean up markdown formatting for display
            import re
            narration_text = re.sub(r'\*\*(.*?)\*\*', r'\1', narration_text)  # Remove bold
            narration_text = re.sub(r'\*(.*?)\*', r'\1', narration_text)      # Remove italics
            narration_text = re.sub(r'`(.*?)`', r'\1', narration_text)        # Remove code
            
            # Split into chunks that fit the screen
            max_chars_per_line = 65  # Slightly reduced for better fit
            max_lines = 8
            ...
            # Take only first max_lines to avoid overflow
            display_lines = lines[:max_lines]
            
            narration_paragraph = Paragraph(
                *display_lines,
                alignment="left",
                font_size=26,  # Slightly smaller
                line_spacing=0.5,
                color=WHITE
            )
            ...
            # Create a simple rectangle without corner_radius (which was causing the error)
            ...
            # Wait based on scene duration from script
            scene_duration = scene.get("length_seconds", 15)
            
            # Calculate remaining time after animations
    ...
    def create_visual_element(self, scene_idx, title):
        ...
        elif "how it works" in title.lower():
            # Process flow
        ...
        elif "applications" in title.lower():
            # Application icons
